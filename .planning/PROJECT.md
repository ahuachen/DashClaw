...
- [x] **Monetization trigger (locked 2026-04-23, Plan 03-03):** Pro tier launches when DashClaw hits **50 verified Claude Code integrations in the wild**. Measured by a SQL query over `action_records` where `agent_id ILIKE 'claude-code%'` excluding internal orgs (`org_default`, `org_demo`), with a 90-day recency window. No time-boxed backstop — the trigger fires when it fires. Committed publicly on [dashclaw.io/pricing](https://dashclaw.io/pricing), [README.md](../README.md), and the launch tweet + HN post (Plan 03-02). Flip path when it fires: `UPDATE organizations SET plan='pro' WHERE id='org_<customer>'` — zero code deploy, `requireTier('pro')` helper already shipped dormant in `app/lib/org.js` (MON-02).
...
| Monetization: free first, paid later — but with a trigger | "Later without a trigger becomes never." Trigger = 50 verified Claude Code integrations in the wild (org_default/org_demo excluded, 90-day recency). Committed publicly at /pricing + README + launch post | ✓ Locked (2026-04-23, Plan 03-03) |
