...
5. **Text-only turns are logged, not attributed.** If an assistant turn produces no tool calls (pure text response), there's no action record to attribute tokens against. The Stop hook writes an `orphan_tokens …` line to `<tempdir>/dashclaw_hook_errors.log` so the spend is visible to ops, but it doesn't land in analytics. If you need this data, consider adding a synthetic "conversation" action type — not built yet.
