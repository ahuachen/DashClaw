...
import { Cpu, Plus, RotateCw, DollarSign, Shield } from 'lucide-react';
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {strategies.map((s) => {
            ...
            return (
              <Link key={s.strategy_id} href={`/model-strategies/${s.strategy_id}`}>
                      ...
                      {fallbacks.length > 0 && (
                        <>
                          ...
                          <div className="text-xs text-zinc-400 font-mono">
                            {fallbacks.map((f, i) => `${f.provider}·${f.model}`).join(' → ')}
                          </div>
                        </>
                      )}
                    </div>
