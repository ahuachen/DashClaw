  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <Card hover={false}>
          <div className="p-4 text-center">
            <div className="text-2xl font-semibold text-emerald-400">{stats.running}</div>
            <div className="text-xs text-zinc-500 mt-1">Running</div>
          </div>
        </Card>
        <Card hover={false}>
          <div className="p-4 text-center">
            <div className="text-2xl font-semibold text-amber-400">{stats.blocked}</div>
      ...
      <div className="mb-6 flex items-center gap-2">
        ...
        {lastUpdated && (
          <span className="ml-auto text-[10px] text-zinc-600">Updated {lastUpdated}</span>
        )}
      </div>
                  ...
                  {filtered.map((session) => (
                    <tr key={session.id} className="hover:bg-white/[0.02] transition-colors group">
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium capitalize ${statusBadge[session.status] || 'bg-zinc-500/20 text-zinc-400'}`}>
                          {session.status}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <Link href={`/sessions/${session.id}`} className="group/name">
                          ...
                          <div className="text-[10px] text-zinc-500 mt-0.5">{session.id}</div>
                        </Link>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs text-zinc-300">{session.branch || '-'}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs text-zinc-300">{session.green_level || '-'}</span>
                      </td>
                      <td className="px-6 py-4">
