...
function parseWorkflowDraft(rawContent) {
...
function resourceList(resources, field) {
  return resources
    .map((resource) => `- ${resource[field]}: ${resource.name}`)
    .join('\n');
}

function promptTemplateList(templates) {
  return templates
    .map((template) => `- ${template.id}: ${template.name} (${template.category || 'general'})`)
    .join('\n');
}

function buildSystemPrompt(resources, preferExistingResources) {
