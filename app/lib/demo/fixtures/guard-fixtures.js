...
function reasoningFor(decision, riskScore, agent, policy) {
  switch (decision) {
    case 'allow':
...
function pickBlockedType(agent) {
...
function metadataFor(decision, riskScore, policy, agent) {
...
for (let i = decisionSlots.length - 1; i > 0; i--) {
  ...
  [decisionSlots[i], decisionSlots[j]] = [decisionSlots[j], decisionSlots[i]];
}

/* ---------- policy selection by decision type ---------- */

function policyForDecision(decision) {
  switch (decision) {
    case 'allow':
      return pick(rnd, policies.filter((p) => p.active));
    case 'block':
      return policies[3]; // Block Dangerous Operations
    case 'warn':
      return policies[0]; // High Risk Action Gate (risk approaching threshold)
    case 'require_approval':
      return policies[1]; // Production Deploy Approval
    default:
...
function riskScoreForDecision(decision) {
  switch (decision) {
    case 'allow':
