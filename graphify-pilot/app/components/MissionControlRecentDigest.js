      ...
      <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="mb-1 flex items-center gap-2">
            ...
            <div className="text-xs font-semibold uppercase tracking-wider text-zinc-300">What Changed In The Last 15 Minutes</div>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {digest.changes.map((change) => (
            <div key={change.id} className="rounded-lg border border-[rgba(255,255,255,0.08)] bg-white/[0.02] px-3 py-3">
              <div className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">{change.label}</div>
              <div className="mb-1 text-2xl font-semibold tabular-nums text-white">{change.count}</div>
              <div className="text-xs leading-5 text-zinc-500">{change.detail}</div>
            </div>
          ))}
        </div>

        <div className="rounded-lg border border-[rgba(255,255,255,0.08)] bg-white/[0.02] px-3 py-3">
          <div className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Recent posture shifts</div>
          {digest.highlights.length === 0 ? (
            <div className="text-xs leading-5 text-zinc-500">No high-signal changes landed in the last 15 minutes.</div>
          ) : (
