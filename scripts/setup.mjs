...
function writeEnvFile(env) {
  const order = [
    ...
    'DASHCLAW_API_KEY', 'DASHCLAW_API_KEY_ORG',
...
async function main() {
  ...
  if (isCloud) {
    ...
    console.log('  2. Import into Vercel, then set these env vars:\n');
    ...
    console.log('     https://github.com/settings/developers, then New OAuth App');
    console.log(`     Callback URL: ${deployUrl}/api/auth/callback/github`);
    console.log('     Add GITHUB_ID + GITHUB_SECRET to Vercel env vars');
    console.log('');
    console.log('  4. Deploy, open the dashboard, and create your workspace.');
    console.log('     After creating a workspace, add one more Vercel env var:');
    console.log('');
    console.log('     DASHCLAW_API_KEY_ORG = (your workspace org ID from the URL or settings)');
    console.log('');
    console.log('     This tells the API key which workspace to write data to.');
    console.log('     Without it, agent data goes to the default org and won\'t appear');
    console.log('     in your workspace dashboard.');
  } else {
